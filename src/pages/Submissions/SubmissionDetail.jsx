...
import { submissionService, gradingService, plagiarismService } from '../../lib/api';
...
export default function SubmissionDetail() {
  ...
  useEffect(() => {
    const p1 = submissionService.getSubmissionById(id).then(setSubmission).catch(() => {});
    const p2 = submissionService.getSubmissionSummary(id).then(setSummary).catch(() => {});
    const p3 = gradingService.getResult(id).then(setGrading).catch(() => {});
    const p4 = plagiarismService.getReport(id).then(setPlagiarism).catch(() => {});
  ...
  return (
    <div>
      <button className={s.back} onClick={() => navigate(-1)}>← Volver</button>
        ...
        <div className={s.section}>
          ...
          <div className={s.scoreRow}>
            <div className={s.scoreCircle}>
              <span className={s.scoreVal}>{grading.final_score ?? '—'}</span>
            ...
            <div className={s.scoreInfo}>
              ...
              {grading.graded_at && (
                <div className={s.infoRow}>
                  ...
                  <span className={s.infoVal}>{new Date(grading.graded_at).toLocaleString('es')}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className={s.section}>
          ...
          <div className={s.plagiarismGrid}>
            <div className={s.plagCard}>
              ...
              <p className={s.plagVal} style={{ color: getColor(plagiarism.internal_similarity) }}>
                {plagiarism.internal_similarity ?? 0}%
              </p>
            </div>
            <div className={s.plagCard}>
              <p className={s.plagLabel}>Similitud externa</p>
              <p className={s.plagVal} style={{ color: getColor(plagiarism.external_similarity) }}>
            ...
            <div className={s.plagCard}>
              ...
              <p className={s.plagService}>{plagiarism.external_service || '—'}</p>
            </div>
          </div>
          {plagiarism.details && (
      ...
      {(submission?.source_code || data.source_code) && (
        <div className={s.section}>
          ...
          <div className={s.codeBlock}>
            <pre className={s.pre}>{submission?.source_code || data.source_code}</pre>
          </div>
        </div>
      )}
    </div>
